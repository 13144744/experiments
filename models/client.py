...
class Client_Net(nn.Module):
    ...
    def download_from_server(self, server):
        ...
        for k in server.W:
            if "hyperbolic_bias" in k:
                self.W[k].data = self.manifold.proj(self.manifold.expmap0(self.manifold.proj_tan0(server.W[k].data.clone(), c=self.c), c=self.c), c=self.c).clone()
            else:
    ...
    def local_train(self, local_epoch):
        ...
        return train_stats["trainingLosses"][0]
    ...
    def compute_weight_update(self, local_epoch):
        ...
        grads_conv = {key: self.W[key].grad for key in self.gconvNames}
        self.convGradsNorm = torch.norm(flatten(grads_conv)).item()
        return train_stats["trainingAccs"][0], train_stats["trainingLosses"][0]
